...
export class GameController {
  ...
  private bindControls(canvas: HTMLCanvasElement) {
    ...
    this.root.querySelector<HTMLButtonElement>("#sidebar-toggle")!.onclick = () =>
      this.toggleSidebar();
    ...
    this.root.querySelector<HTMLButtonElement>("#close-pattern-library")!.onclick = () =>
      patternDialog.close();
    patternDialog.onclick = (event) => {
    ...
    for (const id of ["width", "height", "cellSize", "speed", "density"] as const) {
      this.root.querySelector<HTMLInputElement>(`#${id}`)!.oninput = async (e) => {
        ...
        this.root.querySelector<HTMLElement>(`#${id}-value`)!.textContent = String(
          id === "density" ? `${v}%` : v,
        );
  ...
  private filterPatterns(query: string) {
    ...
    for (const button of this.root.querySelectorAll<HTMLButtonElement>("[data-pattern]")) {
      ...
      const activeCategory =
        this.root.querySelector<HTMLButtonElement>(".category-chip.is-active")?.dataset.category ??
        "all";
  ...
  private selectPattern(patternId: string) {
    ...
    this.root.querySelector<HTMLElement>("#pattern-mode")!.textContent =
      `Placing ${pattern.name}. Click the field, or press Esc.`;
  ...
  private toggleFromMouse(e: MouseEvent) {
    const [x, y] = this.renderer.cellFromEvent(e);
    this.session.toggleCell(x, y);
  }
  private paintFromMouse(e: MouseEvent) {
    const [x, y] = this.renderer.cellFromEvent(e);
    this.session.setCell(x, y, true);
  }
  ...
  private placePattern(pattern: LifePattern, centerX: number, centerY: number) {
    ...
    for (const [x, y] of pattern.cells) {
      ...
      if (
        targetX >= 0 &&
        targetY >= 0 &&
        targetX < this.config.width &&
        targetY < this.config.height
      ) {
  ...
  private template() {
    ...
    const categoryButtons = categories
      .map(
        ([id, label], index) =>
          `<button type="button" class="category-chip${index === 0 ? " is-active" : ""}" data-category="${id}" aria-pressed="${index === 0}">${label}</button>`,
      )
      .join("");
    ...
    return `<section class="shell">
      <aside id="control-panel" class="panel" aria-label="Simulation controls">
        <section class="brand-card" aria-label="Connected Loops Labs Game of Life Statistics">
          ...
          <a class="github-link" href="https://github.com/mustafamohsen/game-of-life" target="_blank" rel="noreferrer" aria-label="View source on GitHub">
            <svg viewBox="0 0 16 16" aria-hidden="true"><path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82A7.6 7.6 0 0 1 8 3.86c.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.01 8.01 0 0 0 16 8c0-4.42-3.58-8-8-8Z" /></svg>
          </a>
        </section>
